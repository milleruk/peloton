...
def plot_metric_with_overlay(workout_df, metric, overlay):
    ...
    is_powerzone = workout_df.get("class_type_ids", [[""]])[0] == "665395ff3abf4081bf315686227d1a51"

    if metric in metric_dict and not (is_powerzone and metric != "Output (watts)"):
        lower_col, upper_col = metric_dict[metric]
        if (
            lower_col in workout_df.columns and 
            upper_col in workout_df.columns and
            pd.api.types.is_numeric_dtype(workout_df[lower_col]) and 
            pd.api.types.is_numeric_dtype(workout_df[upper_col]) and
            not workout_df[lower_col].dropna().empty and 
            not workout_df[upper_col].dropna().empty
        ):
    ...
    if overlay == "music":
        if "title" in workout_df.columns and "artist" in workout_df.columns:
            ...
            for _, row in songs.iterrows():
                color = next(colours)
                ax.axvspan(row["start"], row["index"], alpha=0.2, color=color)
    ...
    instructor = workout_df.get("instructor", ["Unknown Instructor"])[0]
    ax.set_title(f"{class_title} — {instructor} — {metric} with {overlay.capitalize()} Overlay")
